...
const analyticsCmd = program.command('analytics').description('Server-side page-view analytics');

analyticsCmd
  .command('recent')
  .description('Show recent page views')
  .option('-l, --limit <n>', 'Max rows', '50')
  .option('-p, --path <prefix>', 'Filter by path prefix')
  .option('-h, --hours <n>', 'Window size in hours', '24')
  .option('--bots', 'Include bot traffic (default: hide)', false)
  .action((opts) => {
    const sinceMs = Date.now() - Number(opts.hours) * 3600_000;
    const raw = convexRun('cli:analyticsRecent', {
      limit: Number(opts.limit),
      sinceMs,
      excludeBots: !opts.bots,
      pathPrefix: opts.path,
    });
    const rows = parseResult(raw) as Array<{
      timestamp: number;
      ip: string;
      country?: string;
      path: string;
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      console.log('\x1b[2mNo page views in this window.\x1b[0m');
      return;
    }

    console.log(
      `\x1b[1m── ${rows.length} views (last ${opts.hours}h${opts.path ? `, path ${opts.path}` : ''}${opts.bots ? ', incl. bots' : ''}) ──\x1b[0m`,
    );
    for (const r of rows) {
      const when = fmt(r.timestamp);
      const flags = [r.isBot ? '\x1b[33mbot\x1b[0m' : '', r.accessTokenHash ? '\x1b[34mtoken\x1b[0m' : '']
        .filter(Boolean)
        .join(' ');
      const ua = r.userAgent.length > 50 ? r.userAgent.slice(0, 50) + '…' : r.userAgent;
      console.log(
        `${when}  \x1b[36m${(r.ip || '—').padEnd(15)}\x1b[0m  ${(r.country ?? '??').padEnd(2)}  \x1b[1m${r.path}\x1b[0m  ${flags}`,
      );
      console.log(`  \x1b[2m${ua}${r.referer ? `  ← ${r.referer}` : ''}\x1b[0m`);
    }
  });

analyticsCmd
  .command('summary')
  .description('Aggregate view stats (totals, uniques, top paths/countries/IPs)')
  .option('-h, --hours <n>', 'Window size in hours', '24')
  .option('--bots', 'Include bot traffic (default: hide)', false)
  .action((opts) => {
    const sinceMs = Date.now() - Number(opts.hours) * 3600_000;
    const raw = convexRun('cli:analyticsSummary', {
      sinceMs,
      excludeBots: !opts.bots,
    });
    const s = parseResult(raw) as {
      totalViews: number;
      uniqueIps: number;
    ...
    console.log(`\x1b[1m── Summary (last ${opts.hours}h${opts.bots ? ', incl. bots' : ''}) ──\x1b[0m`);
    console.log(`Total views: \x1b[32m${s.totalViews}\x1b[0m`);
    console.log(`Unique IPs:  \x1b[32m${s.uniqueIps}\x1b[0m\n`);

    const printList = <K extends string>(title: string, items: Array<Record<string, any> & { count: number }>, key: K) => {
      console.log(`\x1b[1m${title}\x1b[0m`);
      if (items.length === 0) {
        console.log('  \x1b[2mnone\x1b[0m');
      } else {
        for (const item of items) {
          console.log(`  ${String(item.count).padStart(5)}  ${item[key]}`);
        }
      }
      console.log();
    };

    printList('Top paths', s.topPaths, 'path');
    printList('Top countries', s.topCountries, 'country');
    printList('Top IPs', s.topIps, 'ip');
  });
