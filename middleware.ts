...
export default function middleware(
  ...
  // No language redirect — the site is Polish-only and `/en/` / `/de/` have
  // never been real routes. The old `/` → `/${lang}/` redirect (a) dropped
  // the query string, killing `gclid` / `utm_*` from Google Ads on the first
  // hop, and (b) routed every non-PL Accept-Language visitor (Google's own
  // ad crawlers included) to a 404 catch-all. Legacy `/pl/*` URLs that may
  // still be in the Google index are handled by `src/routes/pl-redirect.tsx`.
  return undefined;
}
