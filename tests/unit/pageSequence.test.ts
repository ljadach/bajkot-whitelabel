...
describe('buildPageSequence', () => {
  it('3-5 produces 25 pages opening with cover, two blanks, title', () => {
    const pages = buildPageSequence('3-5');
    expect(pages).toHaveLength(25);
    expect(pages[0].kind).toBe('cover');
    expect(pages[1].kind).toBe('blank');
    expect(pages[2].kind).toBe('blank');
    expect(pages[3].kind).toBe('title');
    // mood_opening is generated by A7 but no longer placed in the PDF
    expect(pages.find((p) => p.kind === 'mood_opening')).toBeUndefined();
    // Last three pages
    expect(pages[22].kind).toBe('mood_closing');
    expect(pages[23].kind).toBe('parent_card');
    expect(pages[24].kind).toBe('colophon');
  });

  it('6-8 produces 29 pages without mood_opening', () => {
  ...
  it('9+ produces 33 pages and includes beat 4b split across two text pages', () => {
