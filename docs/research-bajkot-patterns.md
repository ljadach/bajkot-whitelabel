...
Reference document for implementing new features in the AITutoro/bajkot codebase.
...
const templateSchemas: Record<PromptTemplate, z.ZodType<any>> = {
  // ... existing
  [PromptTemplate.MyNewSystem]: z.object({
...
const myTable = defineTable({
  ...
  status: v.union(                  // Enum as union of literals
    v.literal('pending'),
...
const docs = await ctx.db
  ...
  .withIndex('by_course_and_page', (q) =>
    q.eq('courseId', courseId).eq('pageIndex', pageIndex)
  )
  .first();
