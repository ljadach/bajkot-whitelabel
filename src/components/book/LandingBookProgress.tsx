...
export function LandingBookProgress() {
  ...
  useEffect(() => {
    trackEvent('progress_viewed', { flow: 'landing', bookOrderId: orderId });
    if (typeof window !== 'undefined') {
      ...
      if (checkout === 'success') {
        ...
        trackEvent('payment_cancelled', { flow: 'landing', bookOrderId: orderId });
      }
    }
  ...
  useEffect(() => {
    ...
    if (!voteViewedRef.current && progress.hasStyleVoteImages && !progress.chosenStyle) {
      voteViewedRef.current = true;
      trackEvent('style_vote_viewed', { flow: 'landing', bookOrderId: orderId });
    }
  }, [progress, phase, orderId]);
  ...
  if (phase === 'dedication') {
    const handleDedicationSubmit = async (dedication: string) => {
      ...
      trackEvent('dedication_submitted', { flow: 'landing', bookOrderId: orderId });
      setPhase('progress');
    };
    const handleDedicationSkip = () => {
      trackEvent('dedication_skipped', { flow: 'landing', bookOrderId: orderId });
      setPhase('progress');
    };
