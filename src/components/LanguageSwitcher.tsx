...
export function LanguageSwitcher({ compact = false }: LanguageSwitcherProps) {
    ...
    return (
      <div ref={dropdownRef} className="relative">
        <button
          ...
          className="flex items-center gap-1 p-2 rounded-lg text-neutral-500 hover:text-neutral-700 hover:bg-neutral-100 transition-colors"
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2 px-3 py-2 rounded-lg border border-neutral-200 hover:border-neutral-300 hover:bg-neutral-50 transition-colors"
      >
        <span className="text-lg">{currentLanguage.flag}</span>
        ...
        <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-neutral-200 py-2 z-50">
          {(
            ...
            <button
              key={code}
              onClick={() => void handleLanguageChange(code)}
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-neutral-50 transition-colors ${code === currentLang ? 'bg-neutral-50' : ''}`}
            >
              <span className="text-xl">{lang.flag}</span>
              <div className="flex-1">
                <div
                  className={`text-sm ${code === currentLang ? 'text-neutral-900 font-medium' : 'text-neutral-700'}`}
                >
                  {lang.nativeName}
                </div>
