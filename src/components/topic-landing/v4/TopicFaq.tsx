...
export function TopicFaq() {
  return (
    <Section track="faq" className="bg-lp-cream-dark" flushTop>
      ...
      <div className="grid gap-2.5 max-w-3xl">
        {LP_FAQ.map((item) => (
          <details
            key={item.q}
            onToggle={(e) => {
              // Which objections people actually open is the cheapest read on
              // what the page fails to answer above the fold.
              if (e.currentTarget.open) trackEvent('lp_faq_opened', { question: item.q });
            }}
