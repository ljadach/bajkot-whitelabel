  ...
  return (
    <section className={`${bgClass} section-spacing`}>
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-neutral-900 mb-8 text-center tracking-tight">{headline}</h2>
        <div className="space-y-5">
          {paragraphs.map((para, index) => {
            ...
            if (hasBullets) {
              ...
              return (
                <div key={index}>
                  {intro.length > 0 && intro[0].trim() && <p className="text-neutral-600 leading-[1.8] text-base sm:text-[17px] mb-3">{emphasizeText(intro.join(' '))}</p>}
                  <ul className="space-y-2 ml-1">
                    {bullets.map((bullet, bi) => (
                      <li key={bi} className="text-neutral-600 leading-[1.8] text-base sm:text-[17px] flex gap-2">
